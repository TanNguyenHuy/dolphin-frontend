...
export default function TransactionList({
    ...
    return (
        <div className="liquid-glass bg-white/50 backdrop-blur-xl rounded-[32px] md:rounded-[40px] p-5 md:p-8 min-w-0 shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-white/80">
            ...
            <div className="flex flex-col gap-4 min-w-0">
                {(enrichedDaily || []).map((row, index) => {
                    ...
                    return (
                        <div 
                            key={row.id || index} 
                            className={`group relative bg-white/70 hover:bg-white backdrop-blur-md rounded-[24px] md:rounded-[28px] p-4 md:p-5 transition-all duration-500 ease-out flex flex-col lg:flex-row gap-5 lg:gap-6 items-start lg:items-center justify-between w-full min-w-0 overflow-hidden ${isMVP ? 'border-2 border-[#FF9500]/50 shadow-[0_8px_24px_rgba(255,149,0,0.15)]' : 'border border-white/80 hover:shadow-[0_15px_40px_rgba(38,208,206,0.12)] hover:border-[#26D0CE]/30'}`}
                        >
                            {/* KHỐI 1: TÊN & THÔNG TIN (Tự động co giãn lấy không gian thừa) */}
                            <div className="flex items-center gap-3 md:gap-4 w-full lg:w-auto lg:flex-1 min-w-[200px]">
                            ...
                            {/* KHỐI 2: DỮ LIỆU ĐỘNG (Tự rớt dòng FLEX-WRAP nếu màn hình hẹp, cam kết không bao giờ đè chữ) */}
                            <div className="flex flex-wrap items-center justify-between lg:justify-end gap-4 md:gap-6 w-full lg:w-auto border-t lg:border-none border-gray-200/60 pt-4 lg:pt-0">
                                
                                {/* 2.1: Nhập - Bán - Còn */}
                                <div className="flex items-center gap-2 shrink-0">
                                    <div className="w-[65px] sm:w-[75px] bg-gray-50/80 border border-gray-200/60 rounded-[16px] py-2 md:py-2.5 text-center group-hover:bg-white transition-colors shadow-sm">
                                    ...
                                    <div className={`w-[65px] sm:w-[75px] border rounded-[16px] py-2 md:py-2.5 text-center transition-colors shadow-sm ${isBanGreater ? 'bg-teal-50/80 border-teal-200/60 group-hover:bg-teal-50' : 'bg-gray-50/80 border-gray-200/60 group-hover:bg-white'}`}>
                                ...
                                {/* 2.2: Tài chính & Nút Action (Sẽ rớt xuống dưới 2.1 nếu bị bóp nhỏ) */}
                                <div className="flex flex-wrap sm:flex-nowrap items-center gap-4 md:gap-6 shrink-0 lg:border-l border-gray-200/60 lg:pl-6 w-full sm:w-auto">
                                    
                                    <div className="flex flex-row sm:flex-col justify-between sm:justify-center w-full sm:w-auto text-left sm:text-right shrink-0">
                                        <div className="flex items-center sm:justify-end gap-2 text-[12px] md:text-[13px]">
                                        ...
                                        <div className="flex items-center sm:justify-end gap-2 text-[11px] md:text-[12px] mt-0.5">
                                    ...
                                    <div className="flex items-center justify-between w-full sm:w-auto gap-4 border-t sm:border-none border-gray-200/60 pt-3 sm:pt-0 shrink-0">
                                        <div className="flex flex-col items-start sm:items-end shrink-0 min-w-[80px]">
                                            ...
                                            <div className={`text-[16px] md:text-[18px] font-black tracking-tight tabular-nums whitespace-nowrap ${parseFloat(row.loi || 0) >= 0 ? 'text-[#1DB2A0]' : 'text-[#FF453A]'}`}>
                                                {formatCurrency(row.loi || 0)}
                                            </div>
                                        </div>

                                        {/* Nút thao tác tối thiểu 44px (w-11 h-11) */}
                                        <div className="flex items-center gap-1.5 bg-gray-50/50 p-1 rounded-full border border-gray-100 shrink-0">
                                            {canEdit && (
