            ...
            <div className="flex flex-col gap-4 min-w-0">
                {(enrichedDaily || []).map((row, index) => {
                    ...
                    return (
                        <div 
                            key={row.id || index} 
                            className={`group relative bg-white/70 hover:bg-white backdrop-blur-md rounded-[24px] md:rounded-[28px] p-4 md:p-5 transition-all duration-500 ease-out flex flex-col xl:flex-row flex-wrap xl:flex-nowrap items-start xl:items-center justify-between gap-4 xl:gap-6 w-full min-w-0 overflow-hidden ${isMVP ? 'border-2 border-[#FF9500]/50 shadow-[0_8px_24px_rgba(255,149,0,0.15)]' : 'border border-white/80 hover:shadow-[0_15px_40px_rgba(38,208,206,0.12)] hover:border-[#26D0CE]/30'}`}
                        >
                            ...
                            {/* 1. Khu vực Thông tin (Chiếm không gian linh hoạt) */}
                            <div className="flex items-center gap-4 flex-1 min-w-[260px] w-full xl:w-auto relative z-10">
                            ...
                            {/* 2. Khu vực Stats: Đã Fix lỗi đè chữ bằng Flex cân bằng */}
                            <div className="flex gap-2 w-full sm:w-auto shrink-0 relative z-10 min-w-[220px]">
                                <div className="flex-1 sm:w-[70px] bg-gray-50/80 border border-gray-200/60 rounded-[16px] py-2 md:py-2.5 text-center group-hover:bg-white transition-colors shadow-sm">
                                ...
                                <div className={`flex-1 sm:w-[70px] border rounded-[16px] py-2 md:py-2.5 text-center transition-colors shadow-sm ${isBanGreater ? 'bg-teal-50/80 border-teal-200/60 group-hover:bg-teal-50' : 'bg-gray-50/80 border-gray-200/60 group-hover:bg-white'}`}>
                            ...
                            {/* 3. Khu vực Tài chính & Hành động */}
                            <div className="flex items-center justify-between xl:justify-end gap-4 w-full xl:w-auto border-t xl:border-none border-gray-200/60 pt-4 xl:pt-0 relative z-10 shrink-0">
                                
                                <div className="flex flex-col justify-center text-left xl:text-right shrink-0">
                                    <div className="flex items-center xl:justify-end gap-2 text-[12px] md:text-[13px]">
                                    ...
                                    <div className="flex items-center xl:justify-end gap-2 text-[11px] md:text-[12px] mt-0.5">
                                ...
                                <div className="flex items-center gap-3 md:gap-4 shrink-0 border-l border-gray-200/60 pl-3 md:pl-4">
                                    <div className="flex flex-col items-start xl:items-end min-w-[80px]">
                                    ...
                                    {/* Action Buttons: Đủ to 44px chuẩn UX */}
                                    <div className="flex items-center gap-1.5 bg-gray-50/50 p-1 rounded-full border border-gray-100">
                                        {canEdit && (
                                            <button onClick={(e) => { e.stopPropagation(); handleStartEdit(row); }} disabled={isProcessingEdit || isProcessingDelete} className="w-9 h-9 md:w-10 md:h-10 flex items-center justify-center text-gray-400 hover:text-[#33A1FD] hover:bg-white hover:shadow-sm rounded-full transition-all active:scale-90" title="Sửa bản ghi">
                                                <Pencil size={16} strokeWidth={2.5}/>
                                            </button>
                                        )}
                                        {canDelete && (
                                            <button onClick={(e) => { e.stopPropagation(); handleDeleteRow(row.id); }} disabled={isProcessingEdit || isProcessingDelete} className="w-9 h-9 md:w-10 md:h-10 flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-white hover:shadow-sm rounded-full transition-all active:scale-90" title="Xóa bản ghi">
