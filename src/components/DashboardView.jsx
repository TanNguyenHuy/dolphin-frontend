...
export default function DashboardView({
    ...
    // =========================================================
    // STATE: QUẢN LÝ BẢNG XEM NHANH (QUICK VIEW) & TẢI CHI TIẾT
    // =========================================================
    ...
    const handleMouseEnterRow = async (session) => {
        ...
        // Tải chi tiết ngầm nếu rê chuột sang đợt mới (tránh gọi API liên tục)
        if (hoveredSessionIdRef.current !== session.id) {
            ...
            try {
                ...
                // Xác nhận người dùng chưa văng chuột đi chỗ khác
                if (hoveredSessionIdRef.current === session.id) {
                    ...
                    // Tính toán ra đầy đủ Lợi Nhuận của từng món
                    const dStats = calculateDetailStats(detailData, balesData, AD_COST_PER_SALE);
                    setQuickViewDetails(dStats.enrichedDaily || []);
    ...
    return (
        <div className="space-y-16 md:space-y-24 animate-fade-in-up pb-24 max-w-[1400px] mx-auto pt-6 relative">
            ...
            <div id="section-danh-sach" className="scroll-mt-[120px] w-full relative">
                <ScrollReveal delay={150}>
                    <div 
                        className="liquid-glass rounded-[32px] md:rounded-[40px] p-6 sm:p-8 md:p-10 border border-white/60 shadow-[0_8px_30px_rgba(0,0,0,0.04)] relative overflow-hidden bg-white/40 w-full xl:w-[70%]"
            ...
            {/* ========================================================= */}
            {/* BẢNG XEM NHANH (QUICK VIEW) - TRONG SUỐT VỚI CHUỘT (pointer-events-none) */}
            {/* ========================================================= */}
            <div 
                className={`hidden xl:block fixed right-6 top-1/2 -translate-y-1/2 w-[370px] z-[100] transition-all duration-[600ms] cubic-bezier(0.16, 1, 0.3, 1) pointer-events-none 
                ...
                {quickViewData && (
                    <div className="flex flex-col gap-3 drop-shadow-[0_20px_40px_rgba(0,0,0,0.15)] max-h-[90vh]">
                        ...
                        <div className="bg-white/95 backdrop-blur-3xl rounded-[24px] p-4 border border-white/60 shrink-0">
                            <h3 className="text-[17px] font-black text-[#1D1D1F] leading-tight mb-1 truncate">
                        ...
                        <div className="bg-white/95 backdrop-blur-3xl rounded-[24px] p-5 border border-white/60 relative overflow-hidden shrink-0">
                            ...
                            <div className="flex items-center gap-2 text-[#1D1D1F] font-black text-[14px] mb-3">
                            ...
                            <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-100/50 rounded-[16px] p-3 mb-3 relative overflow-hidden">
                                ...
                                <div className="text-[24px] font-black text-[#663C00] relative z-10 leading-none">
                                    {formatCurrency((quickViewData.so_tien_cua_kien || 0) + (quickViewData.computedGiatUi || 0))} <span className="text-[15px] opacity-70">đ</span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2 text-[12px] font-bold">
                                <div className="flex justify-between items-center text-gray-500 border-b border-gray-100 pb-1.5">
                                    <span className="uppercase text-[9px] tracking-wider">Phí giặt ủi (Auto 4%)</span>
                                    <span className="text-[#1D1D1F]">{formatCurrency(quickViewData.computedGiatUi || 0)}</span>
                                </div>
                                <div className="flex justify-between items-center text-gray-500">
                                    <span>Tổng Hàng Nhập</span>
                                    <span className="text-[#1D1D1F] text-[13px]">{quickViewData.tong_sl_nhap || 0}</span>
                                </div>
                                <div className="flex justify-between items-center text-gray-500">
                                    <span>Tổng Đã Bán</span>
                                    <span className="text-teal-600 text-[13px]">{quickViewData.tong_sl_ban || 0}</span>
                                </div>
                                <div className="flex justify-between items-center text-gray-500 pt-1.5 border-t border-gray-100">
                            ...
                            <div className="flex flex-col gap-2">
                                {isLoadingDetails ? (
                                    <div className="flex flex-col items-center justify-center py-4 gap-2">
                                        ...
                                        <span className="text-[10px] text-gray-400 font-bold">Đang tính lợi nhuận...</span>
                                    </div>
                                ) : quickViewDetails.length === 0 ? (
                                    <div className="flex items-center justify-center py-4 text-[11px] font-bold text-gray-400">
                                        Chưa có sản phẩm nào
                                    </div>
                                ) : (
                                    <>
                                        {/* Cắt ra 4 sản phẩm đầu tiên để tránh bị dài quá tràn màn hình */}
                                        {quickViewDetails.slice(0, 4).map((item, idx) => (
                                            <div key={idx} className="bg-white border border-gray-100 rounded-[16px] p-2.5 shadow-sm">
                                                <div className="flex justify-between items-start mb-1.5">
                                                    <div className="font-black text-[12px] text-[#1D1D1F] truncate pr-2 flex-1">
                                                        {item.stt}. {item.ten_san_pham}
                                                    </div>
                                                    <div className={`text-[11px] font-black shrink-0 ${item.loi >= 0 ? 'text-[#1DB2A0]' : 'text-rose-500'}`}>
                                                        {item.loi >= 0 ? '+' : ''}{formatCurrency(item.loi)}đ
                                                    </div>
                                                </div>
                                                <div className="flex justify-between items-center text-[10px] font-bold text-gray-500">
                                                    <div className="flex gap-1">
                                                        <span className="bg-gray-50 px-1 rounded border border-gray-100">N: {item.sl_nhap}</span>
                                                        <span className="bg-teal-50 text-teal-600 px-1 rounded border border-teal-100">B: {item.so_luong || 0}</span>
                                                        <span className="bg-orange-50 text-orange-600 px-1 rounded border border-orange-100">C: {item.sl_con}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                        {quickViewDetails.length > 4 && (
                                            <div className="text-center text-[10px] font-bold text-gray-400 mt-1">
                                                + {quickViewDetails.length - 4} sản phẩm khác...
                                            </div>
                                        )}
                                    </>
                                )}
                        ...
                        {/* THẺ 4: TỔNG KẾT HIỆU SUẤT */}
                        <div className="bg-white/95 backdrop-blur-3xl rounded-[24px] p-5 border border-white/60 shrink-0">
                            <div className="flex justify-between items-center mb-3">
                                <span className="font-black text-[14px] text-[#1D1D1F]">Tổng kết Hiệu suất</span>
                                <span className="text-[10px] font-bold bg-gray-100 text-gray-500 px-2 py-1 rounded-lg">Gọn</span>
                            </div>

                            <div className="space-y-2.5">
                                <div className="flex justify-between items-center text-[12px] bg-gray-50/50 p-2 rounded-[12px]">
                                    <span className="font-bold text-gray-500 flex items-center gap-1.5"><TrendingUp size={13} className="text-blue-500"/> Doanh thu</span>
                                    <span className="font-black text-[#1D1D1F]">{formatCurrency(quickViewData.tong_doanh_thu || 0)}đ</span>
                                </div>
                                <div className="flex justify-between items-center text-[12px] bg-gray-50/50 p-2 rounded-[12px]">
                                    <span className="font-bold text-gray-400 flex items-center gap-1.5"><Package size={13} className="text-gray-400"/> Ước tính Vốn tồn</span>
                                    <span className="font-bold text-gray-500">{formatCurrency(quickViewData.tong_tien_ton_computed || 0)}đ</span>
                                </div>
                                <div className="flex justify-between items-center pt-2.5 px-1 border-t border-gray-100">
                                    <span className="font-black text-gray-600 uppercase tracking-widest text-[10px] flex items-center gap-1.5"><Activity size={13} className={quickViewData.realProfit >= 0 ? 'text-teal-500' : 'text-rose-500'}/> LỢI NHUẬN</span>
                                    <span className={`text-[18px] font-black ${quickViewData.realProfit >= 0 ? 'text-[#1DB2A0]' : 'text-rose-600'}`}>
