...
export default function DashboardChart({ enrichedSessions, dashboardProfit }) {
    ...
    // Lọc dữ liệu theo năm và định dạng cho Biểu đồ
    const chartData = useMemo(() => {
        const filtered = enrichedSessions.filter(s => {
            if (!s.actual_start_date) return false;
            return new Date(s.actual_start_date).getFullYear() === selectedYear;
        });

        // Đảo ngược mảng để vẽ từ cũ tới mới (trái qua phải)
        ...
        <div className="bg-white/90 backdrop-blur-xl border border-white/60 rounded-[32px] p-6 md:p-8 shadow-sm w-full h-full flex flex-col">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <div>
                    ...
                    <div className="text-[12px] font-bold text-gray-500 mt-1">Tổng Lợi nhuận năm: <span className="text-[#1D1D1F] text-[14px]">{formatCurrency(dashboardProfit)}đ</span></div>
                </div>

                {/* Nút chọn năm (giống Monthly của mẫu) */}
                <div className="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-full p-1 shadow-inner">
                    <button onClick={() => setSelectedYear(y => y - 1)} className="p-1.5 hover:bg-white rounded-full transition-all text-gray-500 hover:shadow-sm">
                        <ChevronLeft size={16} strokeWidth={3} />
                    </button>
                    <span className="text-[13px] font-black text-[#1D1D1F] w-[50px] text-center">
                        {selectedYear}
                    </span>
                    <button onClick={() => setSelectedYear(y => y + 1)} className="p-1.5 hover:bg-white rounded-full transition-all text-gray-500 hover:shadow-sm">
            ...
            <div className="h-[280px] w-full mt-auto">
                {chartData.length === 0 ? (
                    <div className="flex items-center justify-center h-full text-gray-400 font-bold text-[13px]">Chưa có dữ liệu chốt sổ trong năm {selectedYear}</div>
                ) : (
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                            ...
                            />
                            <YAxis 
                                hide={true} // Ẩn cột Y đi cho giống thiết kế hiện đại
                            ...
                            <Tooltip 
                                ...
                                content={({ active, payload }) => {
                                    if (active && payload && payload.length) {
                                        return (
                                            <div className="bg-[#1D1D1F] text-white p-3 rounded-[12px] shadow-lg border border-gray-700">
                            ...
                            <Area 
                                ...
                                activeDot={{ r: 6, strokeWidth: 0, fill: '#1D1D1F' }}
                            />
                        </AreaChart>
                    </ResponsiveContainer>
                )}
