    ...
    return (
        <div className="animate-fade-in-up pb-20">
            ...
            <div className="space-y-4">
                {users.map(u => {
                    // ĐÃ KHÔI PHỤC BIẾN: Nguyên nhân gây lỗi trắng trang lúc nãy
                        ...
                        <div key={u._id} className={`liquid-glass rounded-[30px] p-6 flex flex-col xl:flex-row xl:items-center justify-between gap-6 border-[3px] transition-all shadow-sm ${isAbandoned ? 'border-gray-200 bg-gray-50/50 opacity-80' : (!u.isApproved && u.role !== 'admin' && u.plan !== 'premium' ? 'border-orange-300 bg-orange-50/50' : 'border-transparent hover:border-gray-200')}`}>
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-3 mb-1">
                                    ...
                                    {/* NHÃN GÓI: Kích thước đồng nhất, Satin mờ nhẹ nhàng */}
                                    {isAbandoned ? (
                                        ...
                                        <span className="w-[100px] h-7 flex items-center justify-center gap-1 rounded-full text-[10px] font-black uppercase tracking-widest border border-violet-200 shadow-sm bg-gradient-to-r from-violet-400 to-fuchsia-400 text-white"><Crown size={12}/> PREMIUM</span>
                                    ) : (
                                        <span className={`w-24 h-7 flex items-center justify-center gap-1 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-sm ${
                                            u.plan === '100k' ? 'bg-gradient-to-r from-amber-200 to-yellow-400 text-yellow-900 border-yellow-300' : 
                                            u.plan === '50k' ? 'bg-gradient-to-r from-slate-200 to-gray-300 text-slate-800 border-slate-300' : 
                                ...
                                <div className="flex flex-wrap items-center gap-3 xl:gap-4 mt-4 xl:mt-0">
                                    {isAbandoned ? (
                                        ...
                                        <>
                                            ...
                                            <button onClick={() => handleApprove(u._id)} className={`px-6 xl:px-8 py-2.5 xl:py-3 rounded-2xl font-black text-[13px] shadow-lg transition-all active:scale-95 ${u.isApproved ? 'bg-gray-100 text-gray-400 border border-gray-200 shadow-none' : 'bg-[#1DB2A0] text-white hover:bg-[#159a8a]'}`}>
                                            ...
                                            <div className="flex flex-wrap gap-4 bg-white/90 p-3 rounded-2xl border border-gray-200 shadow-inner">
                                                {/* ĐÃ KHÔI PHỤC: Ẩn/Hiện Checkbox theo Gói */}
                                                {u.plan === 'premium' && (
                                                    <>
