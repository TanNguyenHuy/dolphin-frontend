...
export default function AdminPanel({ setView, authUser }) {
    ...
    // ĐÃ KHÔI PHỤC CHỨC NĂNG SỬA HẠN
    ...
    // ĐÃ KHÔI PHỤC CHỨC NĂNG CẤM WEB
                        ...
                        <div key={u._id} className="bg-white rounded-[24px] p-6 flex flex-col xl:flex-row xl:items-center justify-between gap-6 border border-gray-100 shadow-sm">
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-3 mb-1">
                                    ...
                                    {/* ĐÃ FIX: Nhãn Satin mờ, kích thước đồng nhất (w-24), bỏ chữ "Gói" */}
                                    <div className={`w-24 h-7 flex items-center justify-center rounded-full text-[10px] font-black tracking-widest border ${
                                        u.plan === 'premium' || u.role === 'admin' ? 'bg-[#D8B4FE] text-white border-[#C084FC]' : 
                                        u.plan === '100k' ? 'bg-[#FDE68A] text-yellow-950 border-[#FCD34D]' : 
                                        u.plan === '50k' ? 'bg-[#E2E8F0] text-slate-800 border-[#CBD5E1]' : 
                                        'bg-[#D7CCC8] text-[#5D4037] border-[#A1887F]'
                                    }`}>
                                        {u.role === 'admin' || u.plan === 'premium' ? 'PREMIUM' : u.plan === '100k' ? 'VVIP' : u.plan === '50k' ? 'VIP' : isAbandoned ? 'CHƯA NẠP' : 'CƠ BẢN'}
                                    </div>
                                </div>
                            ...
                            {u.role !== 'admin' && (
                                <div className="flex flex-wrap items-center gap-3 xl:gap-4 mt-4 xl:mt-0">
                                    ...
                                    {/* ĐÃ KHÔI PHỤC NÚT DUYỆT */}
                                    {!u.isApproved && <button onClick={() => handleApprove(u._id)} className="bg-teal-500 text-white px-5 py-2 rounded-xl text-[11px] font-black shadow-md active:scale-95 transition-all">DUYỆT</button>}
                                    ...
                                    <select className="bg-white border rounded-xl px-3 py-2 text-[11px] font-bold outline-none cursor-pointer" value={u.plan || '10k'} onChange={(e) => handleChangePlan(u._id, e.target.value)}>
                                    ...
                                    {/* ĐÃ KHÔI PHỤC MENU CẤM WEB */}
                                    <select className={`text-[11px] font-bold px-3 py-2 rounded-xl outline-none cursor-pointer border transition-colors ${isRestricted ? 'bg-red-50 text-red-600 border-red-200' : 'bg-white text-gray-600 border-gray-200'}`} onChange={(e) => handleRestrict(u._id, e.target.value)} value={restrictStatus}>
                                        <option value="0">Web: Mở</option>
                                    ...
                                    <button onClick={() => handleDeleteUser(u._id)} className="p-2 text-red-500 bg-red-50 rounded-xl hover:bg-red-500 hover:text-white transition-colors active:scale-95"><Trash2 size={16}/></button>
                                </div>
                            )}
                        </div>
                    )
