...
import { Mail, Lock, User, Eye, EyeOff, RefreshCw, KeyRound, Crown, Star, Check, ChevronLeft, ChevronRight, QrCode, Clock, ArrowRight, Image as ImageIcon, UploadCloud } from 'lucide-react';
...
export default function Auth({ onLoginSuccess }) {
    ...
    const [billBase64, setBillBase64] = useState(null); // Lưu ảnh bill chuyển khoản
    ...
    const handleAuth = async (e) => {
        ...
        try {
            ...
            if (isRightPanelActive) { setRegisteredEmail(formData.email); setStep('pricing'); }
            else {
                    ...
                    <div className="absolute top-0 left-0 w-full md:w-1/2 h-full flex flex-col items-center justify-center p-8 md:p-12 z-20 bg-white">
                        <form onSubmit={handleAuth} className="w-full text-center">
                            ...
                            <div className="space-y-3 mb-6">
                                {isRightPanelActive && <input required className="w-full bg-gray-100 p-3.5 rounded-xl outline-none" placeholder="Tên hiển thị" onChange={e => setFormData({...formData, name: e.target.value})} />}
                                <input required className="w-full bg-gray-100 p-3.5 rounded-xl outline-none" type="email" placeholder="Email" onChange={e => setFormData({...formData, email: e.target.value})} />
                                <input required className="w-full bg-gray-100 p-3.5 rounded-xl outline-none" type="password" placeholder="Mật khẩu" onChange={e => setFormData({...formData, password: e.target.value})} />
                            </div>
                ...
                <div className={`absolute inset-0 bg-white z-[60] flex flex-col items-center justify-center p-6 transition-transform duration-700 ${step === 'qr' ? 'translate-x-0' : 'translate-x-full'}`}>
                    <div className="flex flex-col lg:flex-row items-center gap-12 w-full max-w-[1000px]">
                        <div className="text-center lg:text-left flex-1">
                            ...
                            <button onClick={() => setStep('pricing')} className="w-full mt-4 text-gray-400 font-bold text-sm underline">Quay lại chọn gói</button>
                        </div>

                        {/* MÃ QR SIÊU TO */}
                        <div className="bg-white border-8 border-gray-100 rounded-[40px] shadow-2xl p-4 w-[350px] lg:w-[420px]">
                            <img src={plans[planIndex].qrImage} className="w-full h-full object-contain rounded-[20px]" alt="QR" />
                        </div>
                    </div>
                </div>
