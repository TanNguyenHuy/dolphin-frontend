...
export default function Auth({ onLoginSuccess }) {
    ...
    // THÊM TRẠNG THÁI GHI NHỚ MẬT KHẨU
    ...
    // Tự động điền email/pass nếu đã từng bấm "Ghi nhớ"
    useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (view === 'LOGIN') {
                ...
                if (res.data.user) {
                    // XỬ LÝ GHI NHỚ MẬT KHẨU KHI LOGIN THÀNH CÔNG
                    if (rememberMe) {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#f0f4f9] p-4 font-sans">
            <div className={`relative w-full max-w-[850px] min-h-[550px] bg-white rounded-[20px] shadow-2xl overflow-hidden transition-all duration-700 ease-in-out ${isRightPanelActive ? 'right-panel-active' : ''}`}>
                ...
                <div className={`absolute top-0 left-0 w-full md:w-1/2 h-full flex flex-col items-center justify-center p-10 transition-all duration-700 ease-in-out z-10 ${isRightPanelActive ? 'translate-x-full opacity-100 z-20' : 'opacity-0 md:opacity-100 pointer-events-none md:pointer-events-auto'}`}>
                    <form onSubmit={step === 1 ? handleSendOTP : handleSubmit} className="flex flex-col items-center justify-center w-full h-full text-center">
                        <h1 className="font-black text-[30px] mb-2 text-[#333]">Tạo Tài Khoản</h1>
                        ...
                        {step === 1 ? (
                            <>
                                {/* TRẢ LẠI BO TRÒN HOÀN TOÀN (rounded-full) */}
                                <div className="bg-[#f0f2f5] w-full rounded-full flex items-center px-5 py-3.5 mb-3">
                                ...
                                <div className="bg-[#f0f2f5] w-full rounded-full flex items-center px-5 py-3.5 mb-6 relative">
                                ...
                                <button type="submit" disabled={isLoading} className="rounded-full bg-[#42d4d2] text-white text-[14px] font-bold py-3.5 px-8 tracking-wider transition-transform active:scale-95 hover:bg-[#3bc2c0] w-full flex justify-center uppercase shadow-md">
                                    {isLoading ? <RefreshCw size={18} className="animate-spin" /> : 'Gửi mã OTP qua Email'}
                                </button>
                            </>
                        ) : (
                            <>
                                <div className="bg-[#f0f2f5] w-full rounded-full flex items-center px-5 py-3.5 mb-5">
                ...
                <div className={`absolute top-0 left-0 w-full md:w-1/2 h-full flex flex-col items-center justify-center p-10 transition-all duration-700 ease-in-out z-20 bg-white ${isRightPanelActive ? 'translate-x-full opacity-0 pointer-events-none' : ''}`}>
                    <form onSubmit={view === 'FORGOT' && step === 1 ? handleSendOTP : handleSubmit} className="flex flex-col items-center justify-center w-full h-full text-center">
                        ...
                        {/* TRẢ LẠI LOGO TRONG SUỐT XỊN XÒ TRONG VIDEO */}
                        <div className="w-[56px] h-[56px] bg-[#42d4d2] rounded-full flex items-center justify-center mb-4 text-white shadow-md">
                            <Fish size={30} />
                        </div>
                        
                        <h1 className="font-black text-[30px] mb-2 text-[#333]">
                        ...
                        {view === 'LOGIN' ? (
                            <>
                                ...
                                <div className="bg-[#f0f2f5] w-full rounded-full flex items-center px-5 py-3.5 mb-4 relative">
                                    <Lock size={18} className="text-gray-400" />
                                    <input required type={showPassword ? 'text' : 'password'} placeholder="Mật khẩu" className="bg-transparent outline-none border-none w-full ml-3 text-[14px] pr-8 text-gray-700" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
                                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-5 text-gray-400 hover:text-gray-600">{showPassword ? <EyeOff size={16}/> : <Eye size={16}/>}</button>
                                </div>

                                {/* NÚT GHI NHỚ MẬT KHẨU ĐÃ TRỞ LẠI */}
                                <div className="flex items-center justify-between w-full px-2 mb-6">
                                    <label className="flex items-center cursor-pointer">
                                        <input type="checkbox" className="mr-2 cursor-pointer w-4 h-4 accent-[#42d4d2]" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} />
                                        <span className="text-[13px] text-gray-500 hover:text-gray-700">Ghi nhớ mật khẩu</span>
                                    </label>
                                    <button type="button" onClick={() => { setView('FORGOT'); setStep(1); setError(''); setSuccessMsg(''); }} className="text-[13px] text-gray-500 hover:text-[#42d4d2] font-medium transition-colors">
                                        Quên mật khẩu?
                                    </button>
                                </div>
                            ...
                            step === 1 ? (
                                <>
                                    ...
                                    <button type="submit" disabled={isLoading} className="rounded-full bg-[#42d4d2] text-white text-[14px] font-bold py-3.5 px-8 tracking-wider transition-transform active:scale-95 hover:bg-[#3bc2c0] w-full flex justify-center uppercase shadow-md mb-4">
                                ...
                                <>
                                    ...
                                    <div className="bg-[#f0f2f5] w-full rounded-full flex items-center px-5 py-3.5 mb-5 relative">
                                        <Lock size={18} className="text-gray-400" />
                ...
                {/* 3. OVERLAY TRƯỢT (NỀN XANH) - CHỈ HIỆN TRÊN MÁY TÍNH */}
                <div className={`hidden md:block absolute top-0 left-1/2 w-1/2 h-full overflow-hidden transition-transform duration-700 ease-in-out z-50 ${isRightPanelActive ? '-translate-x-full' : ''}`}>
                    <div className={`bg-gradient-to-r from-[#42d4d2] to-[#3bc2c0] relative -left-full h-full w-[200%] transition-transform duration-700 ease-in-out ${isRightPanelActive ? 'translate-x-1/2' : ''}`}>
                        ...
                        <div className={`absolute top-0 flex flex-col items-center justify-center w-1/2 h-full px-12 text-center text-white transition-transform duration-700 ease-in-out ${isRightPanelActive ? 'translate-x-0' : '-translate-x-[20%]'}`}>
                            <h1 className="font-black text-[32px] mb-4">Mừng Trở Lại!</h1>
                            <p className="text-[14px] font-medium tracking-wide leading-relaxed mb-8">
                                Đăng nhập ngay để tiếp tục quản lý dòng tiền của bạn.
                            </p>
                            <button onClick={() => togglePanel(false)} className="rounded-full border border-white bg-transparent text-white text-[13px] font-bold py-3 px-12 tracking-widest uppercase transition-transform active:scale-95 hover:bg-white hover:text-[#42d4d2]">
                                ĐĂNG NHẬP
                            </button>
                        </div>
                        ...
                        <div className={`absolute top-0 right-0 flex flex-col items-center justify-center w-1/2 h-full px-12 text-center text-white transition-transform duration-700 ease-in-out ${isRightPanelActive ? 'translate-x-[20%]' : 'translate-x-0'}`}>
                            <h1 className="font-black text-[32px] mb-4">Chào Người Mới!</h1>
                            <p className="text-[14px] font-medium tracking-wide leading-relaxed mb-8">
                                Đăng ký để sử dụng không gian quản lý tài chính thông minh nhất.
                            </p>
                            <button onClick={() => togglePanel(true)} className="rounded-full border border-white bg-transparent text-white text-[13px] font-bold py-3 px-12 tracking-widest uppercase transition-transform active:scale-95 hover:bg-white hover:text-[#42d4d2]">
                                TẠO TÀI KHOẢN
                            </button>
                        </div>
